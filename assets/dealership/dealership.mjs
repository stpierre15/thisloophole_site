...
let step=0,selected=null,showDimensions=false,current=null,visibleCount=36,listSort='default';
...
const resultSortOptions=[['default','Original order'],['price-asc','Price · low to high'],['price-desc','Price · high to low'],['seats-desc','Seats · most first'],['cargo-desc','Cargo · most first'],['towing-desc','Towing · most first'],['length-asc','Length · shortest first'],['length-desc','Length · longest first'],['mpg-desc','Gas / hybrid MPG · most first'],['mpge-desc','Electric MPGe · most first']];
function sortedCandidates(data){
 const rows=data.candidates.map((car,index)=>({car,index}));
 ...
 rows.sort((a,b)=>{
  if(data.mode==='filters'){
   const uncertain=Number(a.car.verificationNeeded.length>0)-Number(b.car.verificationNeeded.length>0);
   if(uncertain)return uncertain;
  }
  const left=a.car[field],right=b.car[field];
  if(left==null||right==null)return left==null?right==null?a.index-b.index:1:-1;
  return (direction==='asc'?left-right:right-left)||a.index-b.index;
 });
...
function resultRow(car,i){
 ...
 const details=showDimensions?`<div class="compare-details"><span>Length ${fmt(car.length,' in')}</span><span>Width ${fmt(car.width,' in')}</span><span>Wheelbase ${fmt(car.wheelbase,' in')}</span><span>Height ${fmt(car.height,' in')}</span><span>Clearance ${fmt(car.groundClearance,' in')}</span></div>`:'';
 return `<article class="compare-row ${selected===car.anonymousId?'chosen':''}" data-car="${html(car.anonymousId)}"><div class="compare-identity"><span class="compare-number">CAR ${letter}</span>${car.anonymousImageUrl?`<img loading="lazy" decoding="async" src="${html(car.anonymousImageUrl)}" alt="Structural concept of car ${letter}" width="100" height="68">`:''}<div><h2>${html(car.bodyStyle)} · ${html(car.powertrain)}</h2><p>${status}</p></div></div><div class="compare-cell" data-label="Price">${money(car.price)}</div><div class="compare-cell" data-label="Seats">${seats}</div><div class="compare-cell" data-label="Cargo">${fmt(car.cargo,' cu ft')}</div><div class="compare-cell" data-label="Towing">${car.towing==null?'—':Number(car.towing).toLocaleString('en-US')+' lb'}</div><div class="compare-cell" data-label="Drive">${fmt(car.drivetrain)}</div><div class="compare-cell" data-label="Efficiency">${efficiency}</div><button class="outline-button compare-choice" type="button" data-choice="${html(car.anonymousId)}" aria-pressed="${selected===car.anonymousId}">${selected===car.anonymousId?'SELECTED ✓':'CHOOSE →'}</button>${details}</article>`;
}
function results(data){
 ...
 const ordered=sortedCandidates(data);
 const indexById=new Map(data.candidates.map((car,index)=>[car.anonymousId,index]));
 app.innerHTML=`<section class="results"><div class="results-head"><div><p class="eyebrow">BEFORE THE BADGE / ${data.mode==='filters'?'FILTERED RESULTS':'QUIZ RESULTS'}</p><h1>${amount}</h1></div><div class="results-tools"><a class="text-link" href="${data.mode==='filters'?'/dealership/':'/dealership/quiz/'}">${data.mode==='filters'?'EDIT FILTERS':'RETAKE QUIZ'} ↗</a><label class="dimension-toggle"><input id="dimensions" type="checkbox" ${showDimensions?'checked':''}>SHOW DIMENSIONS</label></div></div><div class="compare-toolbar"><p>COMPARE THE SPECS · BRAND REVEALED AFTER CHOICE</p><label>Sort by <select id="result-sort">${resultSortOptions.map(([value,label])=>`<option value="${value}" ${listSort===value?'selected':''}>${label}</option>`).join('')}</select></label></div><div class="compare-list"><div class="compare-heading"><span>ANONYMOUS VEHICLE</span><span>PRICE</span><span>SEATS</span><span>CARGO</span><span>TOWING</span><span>DRIVE</span><span>EFFICIENCY</span><span></span></div>${ordered.slice(0,visibleCount).map(car=>resultRow(car,indexById.get(car.anonymousId))).join('')}</div>${data.candidates.length>visibleCount?`<button id="more-results" class="outline-button more-results" type="button">SHOW ${Math.min(36,data.candidates.length-visibleCount)} MORE <span>↓</span></button>`:``}<p class="caption">${data.mode==='filters'?'CONFIRMED MATCHES SATISFY EVERY SELECTED FILTER WITH KNOWN VALUES. POSSIBLE MATCHES HAVE UNVERIFIED FIELDS AND ARE LABELED; THEY MAY NOT QUALIFY. ONE POWERTRAIN IS SHOWN PER MODEL. SEATING CAPACITY MAY REQUIRE A SPECIFIC TRIM. ':'QUIZ RESULTS SHOW UP TO TEN MODEL FAMILIES. '}UNKNOWN VALUES SORT LAST. AVAILABLE STRUCTURAL VISUALS ARE CONCEPTUAL, NOT MANUFACTURER CAD; ROWS WITHOUT A VERIFIED RENDER SHOW SPECS ONLY.</p></section><div class="lockbar"><strong>Which one would you buy?</strong><button id="lock" class="button" type="button" ${selected?'':'disabled'}>LOCK IT IN <span>→</span></button></div>`;
