...
let step=0,selected=null,showDimensions=false,current=null,visibleCount=36;
...
function card(car,i){
 const size=car.length==null?1:Math.min(1,car.length/230);
 const traits=[spec('Starting MSRP',money(car.price)),spec(car.powertrain==='Electric'?'MPGe':'Combined MPG',fmt(car.powertrain==='Electric'?car.mpge:car.mpg)),spec('Seats',car.seats==null?'—':car.seatingVaries?`Up to ${html(car.seats)}`:fmt(car.seats)),spec('Cargo',fmt(car.cargo,' cu ft')),spec('Towing',fmt(car.towing,' lb')),spec('Drivetrain',fmt(car.drivetrain))];
 ...
 return `<article class="card ${selected===car.anonymousId?'chosen':''}" data-car="${html(car.anonymousId)}"><div class="card-top"><span>CAR ${letter}</span><span class="match">${current?.mode==='filters'?(car.verificationNeeded.length?'POSSIBLE · VERIFY SPECS':car.dataStatus==='fully-verified'?'FULLY VERIFIED':'SPECS MATCH · ART PENDING'):`${html(car.matchPercent)}% FIT`}</span></div><div class="image-stage">${car.anonymousImageUrl?`<img class="structural" loading="${i<3?'eager':'lazy'}" decoding="async" style="--relative-length:${size}" src="${html(car.anonymousImageUrl)}" alt="Anonymous structural concept of car ${letter}" width="1536" height="1024">`:`<div class="no-art" role="img" aria-label="No verified structural image available"><strong>STRUCTURAL IMAGE<br>PENDING</strong><small>SPECIFICATIONS ONLY · NO FABRICATED RENDER</small></div>`}</div><h2 class="spec-type">${html(car.bodyStyle)} · ${html(car.powertrain)}</h2><div class="specs">${traits.join('')}</div>${dims}<div class="notes"><h3>${current?.mode==='filters'?'MATCHED NEEDS':'WHY IT FITS YOU'}</h3><ul>${car.fitReasons.map(x=>`<li>${html(x)}</li>`).join('')||'<li>Meets your hard requirements</li>'}</ul></div><div class="notes trade"><h3>${car.weaknesses.length?'TRADEOFF':'DATA STATUS'}</h3><ul>${(car.weaknesses.length?car.weaknesses:['Some specifications remain unverified; check after reveal']).slice(0,2).map(x=>`<li>${html(x)}</li>`).join('')}</ul></div><button class="outline-button choice" type="button" data-choice="${html(car.anonymousId)}" aria-pressed="${selected===car.anonymousId}">${selected===car.anonymousId?'SELECTED ✓':"I'D BUY THIS ONE"}</button></article>`;
}
