...
function sortedCandidates(data){
 ...
 const [key,direction]=listSort.split('-');const field={price:'price',cost:'fiveYearCost',seats:'seats',cargo:'cargo',towing:'towing',length:'length',mpg:'mpg',mpge:'mpge'}[key];
 rows.sort((a,b)=>{
  ...
  const left=key==='cost'?a.car.fiveYearCost?.total:a.car[field],right=key==='cost'?b.car.fiveYearCost?.total:b.car[field];
...
function resultRow(car,i){
 ...
 const costCell=cost?`<strong>≈${money(cost.total)}</strong><small>national estimate</small>`:'<span class="cost-unavailable">Price needed</span>';
