...
export function filterVehicles(input,cars=catalog){
  ...
  const ranked=cars.map(car=>({car,verificationNeeded:assessFilters(car,f)})).filter(row=>row.verificationNeeded!==null)
  ...
  for(const {car,verificationNeeded} of ranked){
    ...
    if(f.towing!=='never'&&car.towingCapacity!=null)reasons.push(`${car.towingCapacity.toLocaleString('en-US')} lb towing capacity`);
...
export function blindCar(candidate,sessionId) {
  ...
  return {
    ...
    anonymousImageUrl:car.anonymousAsset?'/assets/dealership/anonymous/'+car.anonymousAsset:null,
    dataStatus:publishedEligible(car)?'fully-verified':'partial',
    verificationNeeded:candidate.verificationNeeded??[],
    bodyStyle:shopperBodyStyle(car),price:car.startingMSRP,powertrain:car.powertrain,fuelType:car.fuelType,seats:availableSeats(car),seatingVaries:!!seating&&availableSeats(car)!==car.seats,
