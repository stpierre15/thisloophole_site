...
import { blindCar, filterVehicles, matchVehicles, validateAnswers, validateFilters } from './dealership-engine.mjs';
...
function blindSession(session,lock){return {sessionId:session.id,createdAt:session.createdAt,mode:session.mode??'quiz',filters:session.mode==='filters'?session.filters:null,locked:!!lock,candidates:session.candidates.map(c=>blindCar(c)),blindChoice:lock?.anonymousId??null,selectionCount:session.candidates.length,confirmedCount:session.mode==='filters'?session.candidates.filter(c=>!c.verificationNeeded?.length).length:null};}
function revealedSession(session,lock,decision){return {...blindSession(session,lock),reveal:session.candidates.map(c=>({anonymousId:c.anonymousId,make:c.car.make,model:c.car.model,year:c.car.year,powertrain:c.car.powertrain,revealedImageUrl:c.car.revealedAsset?'/api/dealership-image?session='+session.id+'&car='+c.anonymousId:null,sourceUrl:c.car.sourceUrl,specSourceUrl:c.car.specSourceUrl??null,seatingSourceUrl:seatingVariantEvidence.get(c.car.id)?.url??seatingEvidence.get(c.car.make+'|'+c.car.model)?.url??null})),decision:decision?{postRevealChoice:decision.anonymousId,changed:decision.changed}:null};}
async function handle(request,context,op){
  const url=new URL(request.url);
  let store;
  const getStore=()=>store??=dealershipStore(context);
  if(op==='start'){
    ...
    if(!await getStore().setNew('sessions/'+id,session))return json({error:'Could not create session'},500);
  ...
  if(op==='filter'){
    ...
    const matches=filterVehicles(filters);
    if(payload.preview===true)return json({count:matches.length,confirmedCount:matches.filter(row=>!row.verificationNeeded.length).length});
    const id=randomUUID();
    const session={id,createdAt:new Date().toISOString(),mode:'filters',filters,candidates:matches.map(row=>({...row,anonymousId:randomUUID()}))};
    if(!await getStore().setNew('sessions/'+id,session))return json({error:'Could not create session'},500);
    return json({sessionId:id,count:session.candidates.length,resultsUrl:'/dealership/results/'+id+'/'},201);
  }
  ...
  const session=await sessionFor(getStore(),id);
  if(!session)return json({error:'Session not found'},404);
  const lock=await getStore().get('locks/'+id);
  if(op==='session'){
    if(!method(request,'GET'))return json({error:'Method not allowed'},405);
    const decision=lock?await getStore().get('decisions/'+id):null;
  ...
  if(op==='lock'){
    ...
    if(!await getStore().setNew('locks/'+id,locked))return json({error:'Choice already locked'},409);
  ...
  if(op==='decision'){
    ...
    if(!await getStore().setNew('decisions/'+id,decision))return json({error:'Decision already recorded'},409);
    return json({changed:decision.changed,postRevealChoice:choice},201);
  }
  return json({error:'Unknown endpoint'},404);
}
export const dealershipHandlers=Object.fromEntries(['start','filter','session','lock','decision','image'].map(op=>[op,async(request,context={})=>{
