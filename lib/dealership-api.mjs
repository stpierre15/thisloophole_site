...
function revealedSession(session,lock,decision){return {...blindSession(session,lock),reveal:session.candidates.map(c=>({anonymousId:c.anonymousId,make:c.car.make,model:c.car.model,year:c.car.year,powertrain:c.car.powertrain,revealedImageUrl:c.car.revealedAsset?'/api/dealership-image?session='+session.id+'&car='+c.anonymousId:null,sourceUrl:c.car.sourceUrl,seatingSourceUrl:seatingVariantEvidence.get(c.car.id)?.url??seatingEvidence.get(c.car.make+'|'+c.car.model)?.url??null})),decision:decision?{postRevealChoice:decision.anonymousId,changed:decision.changed}:null};}
