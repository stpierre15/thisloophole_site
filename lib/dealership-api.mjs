...
function blindSession(session,lock){return {sessionId:session.id,createdAt:session.createdAt,mode:session.mode??'quiz',filters:session.mode==='filters'?session.filters:null,locked:!!lock,candidates:session.candidates.map(c=>blindCar(c)),blindChoice:lock?.anonymousId??null,selectionCount:session.candidates.length};}
...
async function handle(request,context,op){
  ...
  if(op==='filter'){
    ...
    if(payload.preview===true)return json({count:matches.length});
    const id=randomUUID();
