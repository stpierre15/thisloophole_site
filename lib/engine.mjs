...
export function normalizePurchase(input, now = new Date(), allowMissingPrice = false) {
  ...
  return {
    ...
    current_price: price(input.current_price, 'price', allowMissingPrice), condition, category, location: 'US',
...
export function evaluatePurchase(p, records, now = new Date()) {
  ...
  if(p.current_price === null) return {
    purchase:p,verdict:null,evaluation_status:'needs_price',headline:'This listing’s current price is unavailable.',
    score:null,score_label:'Unable to evaluate',dimensions:['Price','Optionality','Benefits','Timing'].map(name=>({name,points:null,reason:'Current offer price is missing.'})),
    confidence:'Low',confidence_reason:'We cannot judge the current purchase without its price. Review the retailer evidence below.',
    potential_savings:null,savings_basis:'No saving calculated without a current price.',better_option:null,
    reasons:['The listing did not expose a source-supported current price. This is a data gap, not a recommendation to wait.'],
    next_actions:['Review any alternative offers below.','Retry the lookup or enter the price shown in your Amazon or retailer session.'],
    loopholes:matches,coverage:'Retailer sources may show different prices by seller, location and account.',methodology_version:'1.2',created_at:now.toISOString()
  };
  ...
  let verdict = null;
  let headline = 'Unable to evaluate this purchase yet.';
  const why = [];
  const actions = [];
  if (poor) {
  ...
  if (p.purchase_timing === 'today' && verdict === null) actions.push('Need it today? Make these checks now; this is not a prediction of an upcoming sale.');
  ...
  const hasEvidence = Boolean(verdict && (comparison || cardSavings));
  ...
  return {
    purchase: p, verdict, evaluation_status: verdict ? 'evaluated' : 'needs_evidence', headline, score, score_label: score === null ? 'Not enough evidence' : score >= 90 ? 'Excellent' : score >= 75 ? 'Good' : score >= 60 ? 'Fair' : score >= 40 ? 'Optimize or wait' : 'Poor', dimensions,
    ...
    coverage: 'US policies · USD · Retailer search has limited coverage. Search-index prices may lag checkout. Unconfirmed amounts are not counted as savings.',
