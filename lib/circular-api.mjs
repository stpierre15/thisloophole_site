...
const provider=new EbayProvider({clientId:process.env.EBAY_CLIENT_ID,clientSecret:process.env.EBAY_CLIENT_SECRET,environment:process.env.EBAY_ENVIRONMENT||'production'});
if(!provider.configured)console.info('Loophole: eBay Browse is not configured. Only labeled examples and the sourced store board are available.');
