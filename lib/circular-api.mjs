...
async function extractor(value){
 ...
 if(new URL(url).hostname.replace(/^www\./,'')==='amazon.com'&&process.env.RAINFOREST_API_KEY){
  ...
  if(result?.product_name){const p=identifyQuery({query:result.product_name,model:result.model||undefined});p.sourceUrl=url;p.identityBasis='metadata';p.identificationNote='Product details were read from the retailer listing through the connected product-data source.';
